...
type LogEntry = FunctionReturnType<
  typeof api.logs.listByRepoWithProjects
>[number];

interface ProjectLogGroupProps {
...
export function ProjectLogGroup({
  projectId,
  projectTitle,
  logs,
  total,
}: ProjectLogGroupProps) {
  const Icon = projectId ? IconLayoutKanban : IconInbox;
  return (
    <Collapsible defaultOpen>
      <CollapsibleTrigger className="motion-base flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium hover:bg-muted/60 sm:gap-2.5 sm:px-4 [&[data-state=open]>.chevron-icon]:rotate-90">
        ...
        <span className="tracking-[-0.01em]">{projectTitle}</span>
