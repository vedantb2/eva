...
/** Wraps a provider git SDK call with timeout, logging, and stale-process cleanup. */
...
export async function createSandbox(
  ...
  return await runLoggedGitStep("createSandbox", details, async () => {
    ...
    // Create does not need the GitHub token at API time (env is a post-create
    // file write). Overlap token fetch with Sandbox.create, then write the env
    // file AFTER onSandboxAcquired so the UI goes active before the
    // first-command boot penalty.
    ...
    try {
      ...
      // since dockerd doesn't survive auto-stop. bootstrapVercelDocker already
      // fast-paths on an already-running daemon (`docker info` check first);
      // the timing wrapper just makes that fast path visible in logs instead
      // of assumed.
...
/**
 * Detects errors meaning the named snapshot is unusable in a way a rebuild
 * would fix (terminal `error` / `build_failed` states from snapshotWorkflow).
 * Caller falls back to creating the sandbox without the snapshot — slower
 * setup, but the run can still proceed.
 */
