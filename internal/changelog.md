...
Verified: backend suite passes (630 tests, including 6 new authorization-contract cases), web suite passes (186 tests), TypeScript checks pass for `@eva/backend` and `@eva/web`, and compiler-check reports no new bailouts. The determinism contract rejected a first cut of the "is this user authorized" query, which was removed rather than granted an exception — the setup page learns it needs the hop from the action that tried to use the token. No dev server, lint, or build was run.
