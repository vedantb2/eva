...
export function TaskDetailModal({
  ...
  const mainContent = (
    <div
      className={
        variant === "panel" ? "flex-1 min-h-0 overflow-y-auto pb-6" : "pb-6"
      }
    >
      ...
      <>
        <div className="flex h-full flex-col border-l border-border">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <h2 className="text-lg font-semibold">{titleElement}</h2>
            <Button
              size="icon"
              variant="ghost"
              className="h-7 w-7"
          ...
          {scheduledBadge && <div className="px-4 pt-2">{scheduledBadge}</div>}
          <div className="flex-1 min-h-0 overflow-y-auto scrollbar px-4 pt-4">
            {mainContent}
          </div>
