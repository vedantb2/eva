...
import { linkLabel } from "./linkChip";
...
/**
 * Inline chip for an external link (Figma today). Renders as a real anchor so
 * clicking opens the URL in a new tab; stops propagation so it never triggers a
 * parent bubble/edit handler.
 */
export function LinkChip({ url, label }: LinkChipProps) {
  ...
  return (
    ...
    >
      <IconBrandFigma className="size-3 shrink-0" />
      {label ?? linkLabel(url)}
    </a>
  );
}
