...
export function PageHeader({ title, headerRight, showBack = false, onBack }: PageHeaderProps) {
  ...
  return (
    <div className="px-2 sm:px-4 py-2 sm:py-3 flex items-center gap-2 sm:gap-3">
      ...
      {title && <h1 className="text-base sm:text-lg font-semibold text-neutral-900 dark:text-neutral-100 truncate">{title}</h1>}
      {headerRight && <div className="ml-auto flex items-center gap-1 sm:gap-2 flex-shrink-0">{headerRight}</div>}
    </div>
  );
}
