...
import { buildProjectBranchName, projectSummaryValidator } from "./helpers";

const PREVIEW_ALLOWED_PHASES = ["active", "completed"] as const;
...
export const startProjectSandbox = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      throw new Error(
        `Project must be in active or completed phase to start sandbox. Current phase: ${project.phase}`,
      );
    }
...
/**
 * Stops the preview sandbox in Daytona. Keeps `sandboxId` so the user
 * can resume the same paused filesystem on next start.
 */
 ...
 * Always flips status, even if Daytona errors.
 */
...
/** Flips project sandbox status from `"stopping"` to `"closed"` after Daytona stop completes. */
...
/** Internal query to fetch a project by ID (used by PTY resolution). */
