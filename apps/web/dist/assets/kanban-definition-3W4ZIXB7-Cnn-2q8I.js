...
import { t as l } from "./chunk-EXTU4WIE-B-MXTPDr.js";
import { n as u, t as d } from "./chunk-MI3HLSF2-pv_-_-Zb.js";
import "./chunk-CVBHYZKI-pVtBnPKQ.js";
import "./chunk-ATLVNIR6-BjpTnV3z.js";
import "./chunk-JA3XYJ7Z-ByUEKWPH.js";
import { a as f, c as p, i as m } from "./chunk-JZLCHNYA-DexENr_S.js";
import { t as h } from "./chunk-FMBD7UC4-DPlkUf_Q.js";
var g = (function () {
...
var _ = g,
  ...
  T = t(function () {
    ...
    return { nodes: t, edges: e, other: {}, config: s() };
  }, `getData`),
  E = t((e, t, n, r, a) => {
    let o = s(),
      l = o.mindmap?.padding ?? c.mindmap.padding;
    switch (r) {
      case D.ROUNDED_RECT:
      case D.RECT:
      case D.HEXAGON:
...
` +
      ...
      (t?.shape && t.shape === `kanbanItem` && (f.shape = t?.shape),
        t?.label && (f.label = t?.label),
        t?.icon && (f.icon = t?.icon.toString()),
        t?.assigned && (f.assigned = t?.assigned.toString()),
        t?.ticket && (f.ticket = t?.ticket.toString()),
        t?.priority && (f.priority = t?.priority));
    }
    let p = C(e);
...
` + t,
      ...
      let g = d.append(`g`);
      g.attr(`class`, `items`);
      let _ = o.nodes.filter((e) => e.isGroup),
        v = 0,
        y = [],
        b = 25;
      for (let e of _) {
      ...
      for (let e of _) {
        ...
        let n = u?.kanban?.sectionWidth || 200,
          r = (-n * 3) / 2 + b,
          i = r,
          a = o.nodes.filter((t) => t.parentId === e.id);
        for (let t of a) {
          if (t.isGroup)
            throw Error(
        ...
        let s = t.cluster.select(`rect`),
          c = Math.max(i - r + 30, 50) + (b - 25);
        s.attr(`height`, c);
      }
      r(
        void 0,
        d,
        u.mindmap?.padding ?? c.kanban.padding,
        u.mindmap?.useMaxWidth ?? c.kanban.useMaxWidth,
      );
    }, `draw`),
  },
  A = t((e) => {
    ...
    let i = t((t, n) => (e.darkMode ? o(t, n) : a(t, n)), `adjuster`);
    for (let t = 0; t < e.THEME_COLOR_LIMIT; t++) {
      let n = `` + (17 - 3 * t);
      r += `
