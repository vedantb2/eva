...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex flex-1">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "group flex flex-1 items-center justify-between py-3 text-sm font-medium transition-colors motion-press active:scale-[0.99] hover:text-foreground [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
