/** Prompt for the `improve-code-structure` catalog entry. */
export const IMPROVE_CODE_STRUCTURE_PROMPT = `Apply this skill to the entire codebase but with a focus on commits made in the past week (if any).

This skill is for when multiple workflows duplicate the same operational logic, when deciding what belongs in actions vs shared services, or when refactoring repeated operational blocks across domain flows. Use when adding new features that share mechanics with existing ones.
