...
async function streamQueryGeneration(question: string, repoId: string, queryId: string) {
...
```typescript
export async function executeMCPQuery(query: string, repoId: string): Promise<unknown[]> {
...
```tsx
export function QueryConfirmation({ query, explanation, onConfirm, onReject }: Props) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardFooter>
        <Button color="primary" onPress={onConfirm}>Run Query</Button>
        <Button variant="flat" onPress={onReject}>Adjust</Button>
      </CardFooter>
    </Card>
  );
}
...
messages: v.array(v.union(
  v.object({ role: v.literal("user"), content: v.string(), timestamp: v.number() }),
  v.object({ role: v.literal("assistant"), content: v.string(), timestamp: v.number() }),
  v.object({ role: v.literal("query"), query: v.string(), explanation: v.string(), timestamp: v.number() }),
  v.object({ role: v.literal("result"), data: v.any(), timestamp: v.number() })
))
```
