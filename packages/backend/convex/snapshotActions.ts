...
export const launchSeedRun = internalAction({
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    lines.push(
      ...
      'command -v claude >/dev/null 2>&1 && command -v codex >/dev/null 2>&1 && [ -d "$(npm root -g)/@anthropic-ai/claude-agent-sdk" ] && [ -d "$(npm root -g)/@cursor/sdk" ] || sudo npm install -g @anthropic-ai/claude-code @anthropic-ai/claude-agent-sdk@0.3.201 @openai/codex agent-browser convex agentation-mcp@1.2.0 @cursor/sdk@1.0.26 || { echo "SEEDRUN-FAILED:agent-clis"; exit 1; }',
