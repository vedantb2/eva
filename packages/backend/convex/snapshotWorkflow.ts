...
export const snapshotBuildWorkflow = workflow.define({
  ...
  handler: async (step, args) => {
    // Step 1: Resolve config, delete old snapshot, POST to Daytona to start the build
    ...
    // Step 2: Poll snapshot state + stream logs until terminal state
    let attempt = 0;
    let state = "";
    ...
    // Step 3: Finalize — if we exhausted polls without terminal state, mark timeout
