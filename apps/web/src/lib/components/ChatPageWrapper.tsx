...
export function ChatPageWrapper({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col w-full">
      {bannerMessage ? (
        <div className="flex w-full items-center gap-2 bg-muted px-3 py-2.5 sm:px-4">
          <IconArchive size={16} className="text-muted-foreground" />
          <span className="text-sm text-muted-foreground">{bannerMessage}</span>
        </div>
      ) : (
        <div className="flex w-full items-center justify-between gap-1 px-3 py-2 sm:gap-2">
          {headerLeft ? (
