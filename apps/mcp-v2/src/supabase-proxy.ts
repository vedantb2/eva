...
export async function registerSupabaseTools(
  ...
  console.log(
    `  Supabase: discovered ${tools.length} tools, registering ${visibleTools.length} read-only tools with prefix "${SUPABASE_PREFIX}"`,
  );
  ...
  console.log(`  Supabase: registered ${visibleTools.length} tools`);
}
