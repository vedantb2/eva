...
- The inbox is now a resizable list/detail split: clicking a notification selects it and shows its full content in a right-hand pane instead of navigating away.
- The detail pane surfaces the complete notification message (previously never shown anywhere in the UI), the source repo and type, and a card linking to the task/project/session/doc the notification is about.
...
The replacement is a single fact. A new `turns` table holds the state machine (`staged` → `launching` → `running` → `finalizing` → terminal), and a turn is running if and only if an open row holds an unexpired `leaseExpiresAt`. Nothing infers, and nothing else gets a vote: `turns.getOpen` answers from row presence alone, and `useTurnInProgress` in `apps/web` is now the one hook the three chat surfaces read. `streamingActivity` survives for display only — its `lastUpdatedAt` no longer means anything, and `streaming.touch` / `internalTouch` are deleted rather than left as a second opinion.

The lease is renewed over the heartbeat the callback already sends. It now carries `TURN_ID`, and the renewal answers with a verdict: `renewed` with a duration, or `terminal` with a reason (`superseded`, `cancelled`, `timeout`, `closed`), which the runner treats as an order to exit. That closes the zombie class structurally — an actor that cannot present the current turn id cannot renew, so it stops touching the sandbox on its next ping whatever else went wrong. Lease durations are phase-aware and reuse the existing staleness policy: 5 minutes idle, 25 with a tool running, 10 while finalizing, and 15 during startup, when no callback exists yet to renew on the turn's behalf. Every grant is capped at `turnStartedAt + RUN_TIMEOUT_MS`, which subsumes the 2-hour `handleStale*` backstops entirely.

Enforcement is level-triggered: two 60-second crons sweep open turns and running runs whose lease has lapsed, and finalize them through the existing `ChatSurfaceAdapter` teardown — cancel the workflow, salvage the streamed bubble, post the alert, interrupt any live process, release the entity, drain the queue. The guarantee is bounded time-to-truth rather than zero bugs: whatever kills the owner, the turn is terminal within the lease grace plus one tick. The sandbox liveness probe survives, but only to choose between "its sandbox was stopped" and "lease expired" in the alert text — it is now structurally incapable of renewing anything. The sandbox deadline extension moved with it: it hangs off renewal, so a dead turn stops keeping its VM alive as a side effect, which the watchdog chain used to do.

Task runs got the same treatment rather than a second mechanism. `agentRuns` carry `leaseExpiresAt` and renew over the same heartbeat — task callbacks carry no turn id, but the streaming entity they already sign is `task-run-<id>` or `task-audit-run-<id>`, so the run is resolved from that and no new wire field exists. The expired-run sweep bounds on `.gt("leaseExpiresAt", 0)`, which is load-bearing: an unset optional field sorts before every number, so without it every run that started before leases existed would look expired and die on the first tick after deploy.

Filesystem markers became crash-only and entity-scoped. The callback writes its own pidfile at `/tmp/eva-runner.<entityIdField>-<entityId>.pid` on boot, because it is the process that knows the fact; the launcher stops writing the shared `/tmp/run-design.pid` guess that every losing launch clobbered. Ready and done files carry the turn id, so a marker left by a previous turn can never be read as this one's.

Deleted: `_taskWorkflow/watchdog.ts`, `_taskWorkflow/livenessProbe.ts`, nine per-surface watchdog wrappers (`handleStale*`, `checkStale*Heartbeat`, `probeStale*Liveness`), the `runStaleChat*` scheduling loop, and the touch mutations. `tests/sessionStallWatchdogContract.test.ts` becomes `tests/turnLeaseContract.test.ts` and pins the five invariants directly, including the two that only fail silently — that the probe cannot renew, and that the reconciler queries take `now` as an argument, since Convex caches a query on its data and never on time.

Verified: `tsc --noEmit` clean in both packages, 566 backend tests with only four pre-existing failures inherited from `main` (`desktopFfmpegInstallContract`, `evaUrls`, `prewarmNeverResurrects`, `snapshotPurgeSafety`), and the callback bundle rebuilt. No chaos run against a live sandbox — killing the runner at each phase and watching the turn reach terminal within three minutes is the check this design deserves, and it has not been done.
