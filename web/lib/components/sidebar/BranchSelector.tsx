...
import { IconGitBranch, IconSelector, IconLoader2, IconRefresh } from "@tabler/icons-react";
...
export function BranchSelector({ owner, repoName, installationId }: BranchSelectorProps) {
  ...
  const fetchBranches = useCallback(async (isSync = false) => {
    if (isSync) {
      setSyncing(true);
    } else {
      setLoading(true);
  ...
  return (
    <div className="flex items-center gap-2">
      <Dropdown>
        <DropdownTrigger>
          <button
            className="flex items-center gap-2 flex-1 px-2 py-2 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors"
            type="button"
          >
      ...
      <button
        ...
        onClick={handleSync}
        disabled={syncing}
        className="p-2 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors disabled:opacity-50"
        title="Sync branches from GitHub"
      >
        <IconRefresh className={`w-4 h-4 text-neutral-500 dark:text-neutral-400 ${syncing ? 'animate-spin' : ''}`} />
      </button>
    </div>
  );
}
