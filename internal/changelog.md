...
- **Startup polish**: The callback now invokes the preinstalled `claude` binary directly instead of `npx @anthropic-ai/claude-code`, and it advances the streaming UI out of "Starting Claude..." as soon as Claude emits its init event instead of waiting for the first assistant content block.
