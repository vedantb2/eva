...
export const getImpactStats = query({
  ...
  handler: async (ctx, args) => {
    const userId = await getCurrentUserId(ctx);
    if (!userId) {
      return { prsShipped: 0, totalSessions: 0, sessionsWithPr: 0, shipRate: 0, tasksCompleted: 0 };
    }
    ...
    const shipRate = totalSessions > 0 ? Math.round((sessionsWithPr / totalSessions) * 100) : 0;
    return { prsShipped: prUrls.size, totalSessions, sessionsWithPr, shipRate, tasksCompleted };
  },
});
...
export const getActiveUsers = query({
  ...
  handler: async (ctx, args) => {
    ...
    const sessions = await ctx.db
      .query("sessions")
      .withIndex("by_repo_and_status", (q) => q.eq("repoId", args.repoId).eq("status", "active"))
      .collect();
    const activeUserIds = new Set<string>();
    for (const session of sessions) {
      if (session.messages.some((m) => m.role === "user" && m.timestamp >= fiveMinAgo)) {
...
export const getActivityTimeline = query({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const buckets: Record<number, { tasks: number; runs: number; sessions: number; prsShipped: number }> = {};
...
export const getLeaderboard = query({
  ...
  handler: async (ctx, args) => {
    ...
    const boards = await ctx.db
      ...
      .withIndex("by_repo", (q) => q.eq("repoId", args.repoId))
      .collect();
    const userStats: Record<string, { tasksCompleted: number; prsCreated: number; sessionsWithPr: number }> = {};
    for (const board of boards) {
      if (!userStats[board.ownerId]) {
        userStats[board.ownerId] = { tasksCompleted: 0, prsCreated: 0, sessionsWithPr: 0 };
      }
      const tasks = await ctx.db
        .query("agentTasks")
        .withIndex("by_board", (q) => q.eq("boardId", board._id))
    ...
    for (const session of filteredSessions) {
      ...
      if (clerkId && session.prUrl) {
        if (!userStats[clerkId]) {
          userStats[clerkId] = { tasksCompleted: 0, prsCreated: 0, sessionsWithPr: 0 };
        }
    ...
    return leaderboard
      .sort((a, b) => b.prsCreated - a.prsCreated || b.tasksCompleted - a.tasksCompleted)
      .slice(0, 5);
  },
});
